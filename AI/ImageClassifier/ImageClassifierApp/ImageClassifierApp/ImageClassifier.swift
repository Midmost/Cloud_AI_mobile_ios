...
class ImagePredictor {
    ...
    static func createImageClassifier() -> VNCoreMLModel {
        ...
        // Custom CoreML 모델 사용
        let imageClassifierWrapper = try? cats_and_dogs(configuration: defaultConfig)
    ...
    private func visionRequestHandler(_ request: VNRequest, error: Error?) {
        ...
        predictions = observations.map { observation in
            guard let number = observation.featureValue.multiArrayValue?[0] else {
                fatalError()
            }
            return Prediction(result: returnLabelString(value: number))
        }
    }
    func returnLabelString(value: NSNumber) -> String {
        let floatValue: Double = value.doubleValue
        if floatValue >= 1 {
            return "강아지 입니다🐶"
        } else {
            return "고양이 입니다🐱"
